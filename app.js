...
function init() {
    ...
    setupAutosaveHandlers(); // Настроить автосохранение
}
...
function loadCurrentWorkoutInProgress() {
    ...
    if (saved) {
        try {
            ...
            // Проверить, что тренировка за сегодня
            ...
            if (currentWorkoutInProgress.date === today) {
                ...
                console.log('✅ Восстановлена незавершенная тренировка:', {
                    day: currentDay,
                    program: currentProgram,
                    exercises: Object.keys(currentWorkoutData).length
                });
            } else {
                // Тренировка не за сегодня - очистить
                console.log('⚠️ Найдена старая тренировка, очищаем');
...
// Автосохранение с debounce и индикатором
...
function debouncedSave() {
    ...
    autosaveTimeout = setTimeout(() => {
        saveCurrentWorkoutInProgress();
    }, 500); // Сохранение через 500мс после последнего изменения
}
...
function changeTheme(theme) {
    // Удалена старая функция - теперь используется toggleTheme()
}
...
function updateSet(exerciseId, setIndex, field, value) {
    ...
    // Автосохранение с debounce при каждом изменении
    debouncedSave();
}
